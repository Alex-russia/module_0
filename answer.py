...
def game_core_v3(number):
    '''Изначально устанавливаем начальное и конечное значение за пределами
        выборки, т.е 0 и 101, среднее значение берем 50 и откидывая по половие
        ищем загаданное число. В откидываемой половине искомого числа нет.'''
...
def game_core_v4(number):
    '''Подход похож на использванный в функции game_core_v3, с той лишь разницей,
    что тут идет разеление области поиска по принципу "Золотое сечение"'''
...
def game_core_v5(number):
    '''Модификация функции game_core_v4, тут "Золотое сечение" отсчитывается
    попеременно, то с начала, то с конца, для этой цели служит флаг countdown'''
...
def game_core_v6(number):
    ...
    start_number = 0
    end_number = 101
    predict_2 = int(start_number + (end_number - start_number) * 0.61803)
    predict_1 = int(end_number - (end_number - start_number) * 0.61803)

    while number != predict_1 and number != predict_2:
        count+=1

        if end_number - start_number == 3:
            predict_2 = start_number + 2
            predict_1 = start_number + 1
        elif number > predict_2:
            start_number = predict_2

            predict_2 = int(start_number + (end_number - start_number) * 0.61803)
            predict_1 = int(end_number - (end_number - start_number) * 0.61803)  
        elif number < predict_1:
            end_number = predict_1
            
            predict_2 = int(start_number + (end_number - start_number) * 0.61803)
            predict_1 = int(end_number - (end_number - start_number) * 0.61803)
        else:
            start_number = predict_1
            end_number = predict_2
...
def game_core_v7(number):
    '''Функция применима для частных случаев (в зависимости от способа расчета попыток))).
    Первоначально делим область поиска на "n" равных частей (ну или почти равных))),
    далее определяем в какой из этих частей находится, угадываемое число, далее 
    применить какую нибудь из функций выше.'''
